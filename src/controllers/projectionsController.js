...
//ESPN and Sleeper have diff. abbrv. of Washington 
...
const getPlayerProjections = async (req, res, next) => {
    try{
        ...
        const p1Opp = normalizeTeam(matchups[p1.team]); 
        const p2Opp = normalizeTeam(matchups[p2.team]); 
        if(!p1Opp || !p2Opp) { return res.status(404).json({ error: 'Matchup not found for one or both players' })};
 
        // Defensive matchup data must come from completed games, not the week
        const defensiveDataWeek = weekInt - 1;
        const defStats = defensiveDataWeek >= 1
            ? await sleeperService.getDefMatchup(seasonInt, defensiveDataWeek)
            : [];
